                ...
                <div class="content-wrapper">
                    ...
                    <div class="col-md-6 grid-margin stretch-card">
                        <div class="card" style="width: 2000px;">
                            ...
                            <div class="card-body">
                                <h4 class="card-title">Ajouter les corrections</h4>
                                <?php
                                if (isset($_GET['corrections']) && !empty($_GET['corrections'])) {
                                    // Afficher le message de succès pour l'ajout d'une annonce
                                    echo '<div class="alert alert-success">' . htmlspecialchars($_GET['corrections']) . '</div>';
                                }
                                if (isset($_GET['corrections88']) && !empty($_GET['corrections88'])) {
                                    // Afficher le message de succès pour l'ajout d'une annonce
                                    echo '<div class="alert alert-danger">' . htmlspecialchars($_GET['corrections88']) . '</div>';
                                }
                                ?>
                                <?php
                                        ...
                                        if ($userRow) {
                                            ...
                                            if ($stmt_professeurs->rowCount() > 0) {
                                                ...
                                                if ($matieres) {
                                                    ...
                                                    echo "<form class='forms-sample' action='ajoutercorrection.php?idUser=$idUser' method='post' enctype='multipart/form-data'>";
                                                    ...
                                                    echo "</div>";
                                                    echo "<label for='exampleInputEmail1'>Titre de correction :</label>";
                                                    ...
                                                    // Ajout du sélecteur pour les TP de la même matière
                                                    echo "<div class='form-group'>";
                                                    echo "<label for='exampleInputUsername1'>Travaux pratiques</label>";
                                                    echo "<select id='tp' name='tp' class='form-control'>";
                                                    echo "<option selected>Choisir un TP</option>";


                                                    // Requête pour récupérer les TP de la même matière
                                                    $sql_tp = "SELECT ID, Titre FROM tp WHERE MatiereID = :codeMatiere";
                                                    $stmt_tp = $conn->prepare($sql_tp);
                                                    $stmt_tp->bindParam(':codeMatiere', $matieres[0]['CodeMatiere']); // On prend la première matière
                                                    $stmt_tp->execute();
                                                    $tps = $stmt_tp->fetchAll(PDO::FETCH_ASSOC);

                                                    foreach ($tps as $tp) {
                                                        echo "<option value='" . $tp['ID'] . "'>" . $tp['Titre'] . "</option>";
                                                    }
                                                    echo "</select>";
                                                    echo "</div>";
                                                    echo "<label for='exampleInputEmail1'>Ajouter une correction :</label>";
                            ...
                            <div class="card" style="background-color: #F5F7FF;">
                                ...
                                <div class="card-body" style="width: 1000px;">
                                    ...
                                    <table class="table custom-table">
                                        <?php
                                        if (isset($_GET['helooo']) && !empty($_GET['helooo'])) {
                                            // Afficher le message de succès pour l'ajout d'une annonce
                                            echo '<div class="alert alert-success">' . htmlspecialchars($_GET['helooo']) . '</div>';
                                        }
                                        ?>
                                        <?php
                                        // Connexion à la base de données avec PDO
                                        ...
                                        try {
                                            ...
                                            $requete_cours_utilisateur = "SELECT * FROM corrections WHERE ProfesseurID = :idUser";
                                            ...
                                            if ($stmt_cours_utilisateur->rowCount() > 0) {
                                                ...
                                                echo "<tr><th>ProfesseurID</th><th>MatiereID</th><th>Titre</th><th>Description</th><th>DatePublication</th><th>Fichier</th><th>TP_ID</th><th>Actions</th></tr>";
                                                while ($row = $stmt_cours_utilisateur->fetch(PDO::FETCH_ASSOC)) {
                                                    // Affichage des données du tableau
                                                    echo "<tr>";
                                                    echo "<td>" . htmlspecialchars($row['ProfesseurID']) . "</td>";
                                                    echo "<td>" . htmlspecialchars($row['MatiereID']) . "</td>";
                                                    ...
                                                    echo "<td>";
                                                    echo "<a href='#' data-toggle='modal' data-target='#modifierModal" . htmlspecialchars($row['ID']) . "'>Modifier</a><br>";
                                                    echo "<a href='supprimercorrection.php?id=" . htmlspecialchars($row['ID']) . "&idUser=" . htmlspecialchars($idUser) . "' onclick=\"return confirm('Êtes-vous sûr de vouloir supprimer cette matière ?');\">Supprimer</a>";

                                                    echo "</td>";
                                                    echo "</tr>";

                                                    // Modal correspondant à chaque événement
                                                    echo "<div class='modal fade' id='modifierModal" . htmlspecialchars($row['ID']) . "' tabindex='-1' role='dialog' aria-labelledby='modifierModalLabel' aria-hidden='true'>";
                                                    ...
                                                    echo "<form action='modifiercorrection.php?idUser=" . htmlspecialchars($idUser) . "' method='post' enctype='multipart/form-data'>";
                                                    echo "<input type='hidden' name='id' value='" . htmlspecialchars($row['ID']) . "'>";
                                                    echo "<label for='nom'>Nouveau nom de la matière:</label><br>";
                                                    echo "<input type='text' id='nom' name='nom' value='" . htmlspecialchars($row['Titre']) . "'><br>";
                                                    echo "<label for='professeur'>Nouveau ID du professeur:</label><br>";
                                                    echo "<input type='text' id='professeur' name='professeur' value='" . htmlspecialchars($row['ProfesseurID']) . "'><br>";
                                                    echo "<label for='matiere'>Nouveau ID de la matière:</label><br>";
                                                    echo "<input type='text' id='matiere' name='matiere' value='" . htmlspecialchars($row['MatiereID']) . "'><br>";
                                                    ...
                                                    echo "<input type='date' id='date' name='date' value='" . htmlspecialchars($row['DatePublication']) . "'><br>";
                                                    ...
                                                    foreach ($tps as $tp) {
                                                        $selected = ($tp['ID'] == $row['TP_ID']) ? 'selected' : '';
                                                        echo "<option value='" . $tp['ID'] . "' " . $selected . ">" . $tp['Titre'] . "</option>";
                                                    }
                                                ...
                                                echo "<p>Aucun enregistrement trouvé pour les tps.</p>";
                                            }
                                        } catch (PDOException $e) {
                                            echo "Erreur : " . $e->getMessage();
                                        }
