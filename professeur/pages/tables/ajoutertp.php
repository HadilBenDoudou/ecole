    ...
    if (isset($_FILES['coursFile']) && $_FILES['coursFile']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['coursFile']['tmp_name'], $uploadPath)) {
            ...
            try {
                ...
                $stmt = $conn->prepare("INSERT INTO tp (ProfesseurID, MatiereID, Titre, Description, DatePublication, Fichier) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt->execute([$professeurID, $matiereID, $titre, $description, $datePublication, $uploadPath]);

                $tp3 = "Ajout avec succes.";
                header("Location: basic-table3.php?idUser=$idUser&tp3=" . urlencode($tp3));
                exit();
