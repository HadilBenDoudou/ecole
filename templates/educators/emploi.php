...
try {
    ...
    $sql_carte_etudiante = "SELECT carte_etudiant FROM utilisateur WHERE iduser = ?";
    ...
    if ($stmt_carte_etudiante->rowCount() > 0) {
        ...
        if ($stmt_classe_etudiant->rowCount() > 0) {
            ...
            $sql_image_emploi_etudiant = "SELECT image_file FROM emplois_etudiant WHERE classe = ? ORDER BY id DESC LIMIT 1";
