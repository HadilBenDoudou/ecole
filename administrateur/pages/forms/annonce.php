                    ...
                    <div class="row">
                        <div class="col-md-6 grid-margin stretch-card">
                            <div class="card" style="width: 200px;">
                                <div class="card-body">
                                    <h4 class="card-title">Ajouter des annonces</h4>
                                    <?php
                                    if (isset($_GET['add_success']) && !empty($_GET['add_success'])) {
                                        // Afficher le message de succès pour l'ajout d'une annonce
                                        echo '<div class="alert alert-success">' . htmlspecialchars($_GET['add_success']) . '</div>';
                                    }
                                    ?>
                                    <script>
                                        // Supprimer la div d'alerte après 5 secondes
                                        setTimeout(function() {
                                    ...
                                    <form action="add_announcement.php?idUser=<?php echo $idUser; ?>" method="post">
                            ...
                            <div class="card" style="background-color: #F5F7FF;">
                                <div class="card-body" style="width: 1000px;">
                                    <table class="table custom-table">
                                        <?php
                                        if (isset($_GET['edit_success']) && !empty($_GET['edit_success'])) {
                                            // Afficher le message de succès pour la modification d'une annonce
                                            echo '<div class="alert alert-success">' . htmlspecialchars($_GET['edit_success']) . '</div>';
                                        }
                                        ...
                                        <tbody>
                                            ...
                                            try {
                                                ...
                                                $sql = "SELECT AnnouncementID,Date, Title, Status, Description FROM Announcements ORDER BY Date DESC";
                                                ...
                                                foreach ($announcements as $announcement) {
                                                    ...
                                                    echo "<td>" . $announcement['Status'] . "</td>";
                                                    echo "<td>" . $announcement['Description'] . "</td>";
                                                    echo "<td>";
                                                    echo "<button class='btn btn-primary btn-sm' data-toggle='modal' data-target='#modifierModal" . $announcement['AnnouncementID'] . "'>Modifier</button>";
                                                    echo "<form method='POST' action='delete_announcement.php?idUser=" . $idUser . "'>";
                                                    echo "<input type='hidden' name='id' value='" . $announcement['AnnouncementID'] . "'>";
                                                    echo "<button type='submit' class='btn btn-danger btn-sm' onclick='return confirm(\"Êtes-vous sûr de vouloir supprimer cette annonce ?\")'>Supprimer</button>";
                                                    echo "</form>";
                                                    echo "</td>";
                                                    echo "</tr>";

                                                    // Modal pour la modification de l'annonce
                                                    echo "<div class='modal fade' id='modifierModal" . $announcement['AnnouncementID'] . "' tabindex='-1' role='dialog' aria-labelledby='modifierModalLabel' aria-hidden='true'>";
                                                    echo "<div class='modal-dialog' role='document'>";
                                                    echo "<div class='modal-content'>";
                                                    echo "<div class='modal-header'>";
                                                    echo "<h5 class='modal-title' id='modifierModalLabel'>Modifier l'annonce</h5>";
                                                    echo "<button type='button' class='close' data-dismiss='modal' aria-label='Close'>";
                                                    echo "<span aria-hidden='true'>&times;</span>";
                                                    echo "</button>";
                                                    echo "</div>";
                                                    echo "<div class='modal-body'>";
                                                    echo "<form method='POST' action='update_announcement.php?idUser=" . $idUser . "'>";
                                                    echo "<input type='hidden' name='id' value='" . $announcement['AnnouncementID'] . "'>";
                                                    echo "<div class='form-group'>";
                                                    echo "<label for='date'>Date</label>";
                                                    echo "<input type='date' class='form-control' id='date' name='date' value='" . $announcement['Date'] . "' required>";
                                                    echo "</div>";
                                                    echo "<div class='form-group'>";
                                                    echo "<label for='title'>Titre</label>";
                                                    echo "<input type='text' class='form-control' id='title' name='title' value='" . $announcement['Title'] . "' required>";
                                                    echo "</div>";
                                                    echo "<div class='form-group'>";
                                                    echo "<label for='status'>Statut</label>";
                                                    echo "<input type='text' class='form-control' id='status' name='status' value='" . $announcement['Status'] . "' required>";
                                                    echo "</div>";
                                                    echo "<div class='form-group'>";
                                                    echo "<label for='description'>Description</label>";
                                                    echo "<textarea class='form-control' id='description' name='description' required>" . $announcement['Description'] . "</textarea>";
                                                    echo "</div>";
