            ...
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-md-8">
                        ...
                        $servername = "localhost"; // Remplacez localhost par l'adresse de votre serveur MySQL si nécessaire
                        $username = "root"; // Remplacez par votre nom d'utilisateur MySQL
                        $password = ""; // Remplacez par votre mot de passe MySQL
                        $dbname = "ecole"; // Remplacez par le nom de votre base de données MySQL

                        try {
                            // Création d'une connexion PDO
                            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);

                            // Configuration de PDO pour qu'il génère des exceptions en cas d'erreurs
                            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

                            // Requête SQL pour récupérer tous les noms de club distincts
                            $club_query = "SELECT DISTINCT formation_id, nom_formation FROM formations";

                            // Exécution de la requête pour récupérer les noms de club
                            $club_stmt = $conn->query($club_query);

                            // Parcours des résultats de la requête pour chaque club
                            while ($club_row = $club_stmt->fetch(PDO::FETCH_ASSOC)) {
                                // Affichage du nom du club
                                echo "<div class='card mb-4'>
                        <div class='card-header bg-primary text-white'>
                            <h3 class='card-title'>{$club_row['nom_formation']}</h3>
                        </div>
                        <div class='card-body'>
                            <div class='table-responsive'>
                                ...
                                // Requête SQL pour récupérer les membres du club spécifié avec le nom et prénom du formateur
                                $membre_query = "SELECT fm.nom, fm.prenom, fm.carte_etudiant, fm.classe, p.nom AS formateur_nom, p.prenom AS formateur_prenom
                        FROM formationmembre fm
                        INNER JOIN professeur p ON fm.formateur_id = p.id
                        WHERE fm.formation_id = {$club_row['formation_id']}";

                                // Exécution de la requête pour récupérer les membres du club avec le nom et prénom du formateur
                                $membre_stmt = $conn->query($membre_query);

                                // Parcours des résultats de la requête pour les membres du club
                                while ($membre_row = $membre_stmt->fetch(PDO::FETCH_ASSOC)) {
                                    echo "<tr>
                                <td>{$membre_row['nom']}</td>
                                <td>{$membre_row['prenom']}</td>
                                <td>{$membre_row['carte_etudiant']}</td>
                                <td>{$membre_row['classe']}</td>
                                <td>{$membre_row['formateur_nom']} {$membre_row['formateur_prenom']}</td>
                            </tr>";
                                }
